...
public class ProviderPayoutService {
    ...
    public ProviderPayout requestPayout(Provider provider) {
        ...
        // 🔒 Weekly withdrawal enforcement
        if (provider.getLastPayoutRequestedAt() != null) {
            LocalDateTime nextAllowedDate = provider.getLastPayoutRequestedAt().plusDays(7);
            if (LocalDateTime.now().isBefore(nextAllowedDate)) {
                throw new RuntimeException("You can request payout only once every 7 days");
            }
        }

        // 🔐 Prevent duplicate INITIATED payout
        boolean alreadyInitiated = payoutRepository.existsByProviderAndStatus(
                provider, PayoutStatus.INITIATED);
        ...
        List<ProviderEarning> available = earningRepository.findAvailableForUpdate(provider);

        if (available.isEmpty()) {
            throw new RuntimeException("No earnings available for withdrawal");
        }

        // ← Include penalty deductions in payout total
        double penaltyTotal = normalize(earningRepository.sumByProviderIdAndStatus(
                provider.getId(), EarningStatus.PENALTY)); // negative value
        ...
        if (total <= 0) {
            throw new RuntimeException("No earnings available after penalty deductions");
        }
        ...
        payout.setPayoutMonth(LocalDate.now().getMonth().name());
        payoutRepository.save(payout);
        notificationService.payoutRequested(provider, total);
        // 🔁 Move AVAILABLE earnings → REQUESTED
        for (ProviderEarning earning : available) {
            earning.setStatus(EarningStatus.REQUESTED);
            earning.setPayout(payout);
        }
        ...
        // 🔁 Move PENALTY earnings → REQUESTED so they don't keep subtracting
        List<ProviderEarning> penalties = earningRepository.findByProviderAndStatus(
                provider, EarningStatus.PENALTY);
        for (ProviderEarning p : penalties) {
            p.setStatus(EarningStatus.REQUESTED);
            p.setPayout(payout);
        }
        earningRepository.saveAll(penalties);
